...
class BaumWelchFilter(Estimator):
    ...
    def update(self):
        ...
        for i in range(self.B_count):
            v = [transition_prob(i, j) for j in range(self.B_count)]
            A.append(v/sum(v))
        v = [transition_prob(i, self.B_count//2) for i in range(self.B_count)]
        pi = v/sum(v)
